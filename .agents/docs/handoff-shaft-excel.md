...
この確認用出力は、`test/今町橋りょう4P(右).sdc` と `test/今町橋りょう4P(C方向･右押し→).ndu`、全4工程、KG対応 `4:1 5:2 6:3`、右押し、周面 `existing-screen` で作成した。モデル未保存の計算確認用。新しい周面見本ではなく、現在の生成側を確認する資料として使う。
