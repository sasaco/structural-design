...
- SDCは行番号ではなく見出し・列名・単位を検証する。奇偶形式は実杭列数へ展開する。Ver.5.2.3の鉛直ばねにある「長期,,短期」・奇偶4見出し・6値という省略形式を限定的に解釈する。加えて、Ver.5.2.1の「長期,短期」3値と杭列見出しを省略した共有値形式では、K1=第2欄、K2=第3欄、Fy=支持力第1欄、Fu=第2欄を実杭列2列または3列へ共有する。どちらも解釈種別と原出典欄をJSONに記録する。
