...
[`scripts/fill_pile_tip_suppot_info.py`](../../scripts/fill_pile_tip_suppot_info.py) は、右SDCの杭先端ばね・支持力を、NDUの `SuppotInfo` へ入力する。調査の根拠・原ファイルの値・フィールド対応は [杭先端の対応調査](snap-pile-tip-suppot-info-mapping.md) に記載した。
