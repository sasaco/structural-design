...
class PortableTests(unittest.TestCase):
    def setUp(self):
        ...
        self.sdc, self.ndu, self.ndt = [self.folder / ("入力データ." + ext) for ext in ("sdc", "ndu", "ndt")]
        self.sdc.write_bytes(sdc_bytes())
        self.ndu.write_bytes(ndu_bytes(empty=True))
        self.ndt.write_bytes(ndt_bytes(empty=True))
    ...
    def test_prepare_does_not_write_and_new_save_preserves_sources(self):
        original = {p: p.read_bytes() for p in (self.sdc, self.ndu, self.ndt)}
        plan = self.plan()
        ...
        self.assertEqual(app.support_values(plan.data, False)[405][0], "100.125")
    ...
    def test_ndt_uses_ndt_current_values_and_preserves_geometry(self):
        self.ndt.write_bytes(ndt_bytes())
        request = replace(self.request, ndt=self.ndt)
        plan = app.prepare(request)
        self.assertIn("K1+=91", plan.rows[0].before)
        self.assertIn("K1+=100.125", plan.rows[0].after)
        original = self.ndt.read_bytes()
        self.assertEqual(plan.data.split(b"SUPPORT")[0], original.split(b"SUPPORT")[0])
        self.assertEqual(plan.data.split(b"END")[-1], original.split(b"END")[-1])

    def test_ndt_wrong_geometry_fails_before_any_write(self):
        self.ndt.write_bytes(self.ndt.read_bytes().replace(b"     3.000", b"     4.000"))
        with self.assertRaises(app.InputError):
            app.prepare(replace(self.request, ndt=self.ndt))
        self.assertEqual(len(list(self.folder.iterdir())), 3)
    ...
    def test_invalid_operations_and_groups_and_direction(self):
        for changes in ({"operations": ()}, {"operations": ("unknown",)},
                        ...
                        {"operations": ("shaft",)}, {"ndt": self.ndt, "operations": ("horizontal",)}):
    ...
    def test_invalid_output_modes_and_suffixes(self):
        ...
        for output, overwrite in ((self.ndu, False), (self.sdc, False), (self.output, True),
                                  (self.folder / "output.ndt", False), (self.folder / "absent/out.ndu", False)):
    ...
    def test_reference_ndu_cannot_be_overwritten_for_ndt(self):
        plan = app.prepare(replace(self.request, ndt=self.ndt))
        with self.assertRaises(app.InputError):
            app.save(plan, self.ndu, overwrite=True)
...
class ProjectIntegrationTests(unittest.TestCase):
    def test_all_four_on_real_model_preserve_unrelated_fields_and_sources(self):
        ...
        self.assertEqual(app.base.parse_ndu(plan.data).fields("JibanShogenInfo98", 7)[1:4],
                         ["58812", "1162.9", "1307.5"])
        self.assertEqual(app.support_values(plan.data, False)[122],
